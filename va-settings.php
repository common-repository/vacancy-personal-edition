			...
			<div id="va-general">
				<form method="post" action="">
					...
					<p>
						...
						<div id="va-require-login" style="display:none;">
							<p><?php _e('Only these roles can use Vacancy','vpe'); ?></p>
							<?php global $wp_roles; ?>
							<ul>
							<?php foreach($wp_roles->roles as $role => $data) : ?>
								<li>
									<input id="<?php echo $role; ?>" type="checkbox" name="va_show_vacancy_for[]" value="<?php echo $role; ?>" <?php if(is_array($this->va_settings['show_vacancy_for'])){if(in_array($role, $this->va_settings['show_vacancy_for'])){echo 'checked';}}else if($role == $this->va_settings['show_vacancy_for']){echo 'checked';} ?>/> <label for="<?php echo $role; ?>"><?php echo $role; ?></label>
									<span class="va-clearer"></span>
								</li>
							<?php endforeach; ?>
							</ul>
...
<script type="text/javascript">
    jQuery(document).ready(function($){
		...
		if($('select[name="va_require_login"]').val() == 'yes'){
			$('#va-require-login').show();
		}
		$('select[name="va_require_login"]').on('change', function(){
			$('#va-require-login').slideToggle('fast');
		});		
