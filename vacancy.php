    ...
    class Vacancy {
        ...
        function __construct(){
            ...
            $this->va_settings = array(
                ...
                'show_vacancy_for' => 'administrator',
            ...
            // CONGRATULATIONS!! VENUES AND LOCATIONS ARE NOW UNLIMITED!!
        ...
        function init(){
            ...
            if(isset($_POST['va_update_settings'])){
                ...
                if(isset($_POST['va_name_label'])){update_option('va_name_label', sanitize_text_field($_POST['va_name_label']));}
                if(isset($_POST['va_email_label'])){update_option('va_email_label', sanitize_text_field($_POST['va_email_label']));}
                if(isset($_POST['va_title_label'])){update_option('va_title_label', sanitize_text_field($_POST['va_title_label']));}
                if(isset($_POST['va_phone_label'])){update_option('va_phone_label', sanitize_text_field($_POST['va_phone_label']));}
                if(isset($_POST['va_reservation_type_label'])){update_option('va_reservation_type_label', sanitize_text_field($_POST['va_reservation_type_label']));}
                if(isset($_POST['va_description_label'])){update_option('va_description_label', sanitize_text_field($_POST['va_description_label']));}
                if(isset($_POST['va_setup_needs_label'])){update_option('va_setup_needs_label', sanitize_text_field($_POST['va_setup_needs_label']));}
                ...
                if(isset($_POST['va_show_vacancy_for'])){ // check other field since this one can be not set
                    if(!empty($_POST['va_show_vacancy_for'])){
                        $vals = $_POST['va_show_vacancy_for'];
                        if(is_array($vals)){
                            foreach($vals as $k => $v){$vals[$k] = sanitize_text_field($v);}
                            update_option('va_show_vacancy_for', $vals);
                        }
                    }else{
                        update_option('va_show_vacancy_for', array(''));
                    }
                }
            ...
            $name_label = get_option('va_name_label');
            if(!empty($name_label)){$this->va_settings['name_label'] = $name_label;}
            $email_label = get_option('va_email_label');
            if(!empty($email_label)){$this->va_settings['email_label'] = $email_label;} 
            $title_label = get_option('va_title_label');
            if(!empty($title_label)){$this->va_settings['title_label'] = $title_label;} 
            $phone_label = get_option('va_phone_label');
            if(!empty($phone_label)){$this->va_settings['phone_label'] = $phone_label;} 
            $reservation_type_label = get_option('va_reservation_type_label');
            if(!empty($reservation_type_label)){$this->va_settings['reservation_type_label'] = $reservation_type_label;} 
            $description_label = get_option('va_description_label');
            if(!empty($description_label)){$this->va_settings['description_label'] = $description_label;} 
            $setup_needs_label = get_option('va_setup_needs_label');
            if(!empty($setup_needs_label)){$this->va_settings['setup_needs_label'] = $setup_needs_label;} 
            $av_needs_label = get_option('va_av_needs_label');
            if(!empty($av_needs_label)){$this->va_settings['av_needs_label'] = $av_needs_label;} 

            $end_time_type = get_option('va_end_time_type');
            if(!empty($end_time_type)){$this->va_settings['end_time_type'] = $end_time_type;} 
            ...
            $end_time_min_length_hr = get_option('va_end_time_min_length_hr');
            if(!empty($end_time_min_length_hr)){$this->va_settings['end_time_min_length_hr'] = $end_time_min_length_hr;} 
            $end_time_min_length_min = get_option('va_end_time_min_length_min');
            if(!empty($end_time_min_length_min)){$this->va_settings['end_time_min_length_min'] = $end_time_min_length_min;} 
            $end_time_max_length_hr = get_option('va_end_time_max_length_hr');
            if(!empty($end_time_max_length_hr)){$this->va_settings['end_time_max_length_hr'] = $end_time_max_length_hr;} 
            $end_time_max_length_min = get_option('va_end_time_max_length_min');
            ...
            $this->va_settings['show_vacancy_for'] = get_option('va_show_vacancy_for');
        ...
        function va_reservation_sortable_columns_filter($columns){
            ...
            $columns['reservation-date'] = 'reservation_date';
            $columns['setup'] = 'setup';
            $columns['start'] = 'start';
            $columns['end'] = 'end';
                ...
                foreach($dates as $date){
                    ...
                    foreach($location_ids as $location_id){
                        ...
                        if(!$location_start && !$location_end){
                            ...
                            $conflicts[$date] = sprintf(__('%1$s %2$s are not available %3$s after %4$s %5$s on %6$s','vpe'),get_the_title($location_id),$this->va_settings['reservation_plural'],'<strong>','</strong>',date('g:i a', strtotime($location_end)),date('l', strtotime($date)));
                            $conflict = true;
                        }else{
        ...
        function va_display_form($atts){
            ...
            if($this->va_settings['require_login'] == 'yes'){
                ...
                $user_roles = $current_user->roles;
                $user_role = array_shift($user_roles);
                if(is_array($this->va_settings['show_vacancy_for'])){
                    if(in_array($user_role, $this->va_settings['show_vacancy_for'])){
                        $can_submit = true;
                    }
                }else if($user_role == $this->va_settings['show_vacancy_for']){
                   $can_submit = true;
                }
            ...
            if($can_submit){
                ...
                <form id="va-day-form" method="post" action="">
                    ...
                    <?php $num_venues = $venues->found_posts; ?>
                    
                    <?php if($venues->have_posts()): ?>
                       <p <?php if($num_venues == 1){echo 'style="display:none;"';} ?>>
                ...
                <?php // scroll to vacancy form if submitted already ?>
                <?php if(isset($_POST['va_date'])) : ?>
                    <script type="text/javascript">
                        jQuery(document).ready(function($){
                            $('body,html').animate({scrollTop:$('#va-day-form').offset().top - 60},800, "easeInOutQuint");
                        });
                    </script>
                <?php endif; ?>
                ...
                <div class="va-lightbox-form">
                    ...
                    <div class="va-end">
                        ...
                        <?php elseif($this->va_settings['end_time_type'] == "fixed") : ?>
                            <label>
                                ...
                                ?>
                                <?php echo $this->va_get_time_select('va_end_time', $end_time, null, true, $end_time, $end_time); ?>
                            </label>
                        <?php else : // min/max ?>
                            <label>
                                <strong><?php printf(__('End %1$s at','vpe'),$this->va_settings['reservation_single']); ?>:</strong> 
                                <?php 
                                    ...
                                    $start_time_min = date('H:i',strtotime("+".$this->va_settings['end_time_min_length_min']." minutes", $start_time_temp));
                                                  
                                    $end_time_temp = strtotime("+".$this->va_settings['end_time_max_length_hr']." hours", strtotime($start_time));
                                    $end_time_max = date('H:i',strtotime("+".$this->va_settings['end_time_max_length_min']." minutes", $end_time_temp));      

                                    $interval = $this->va_settings['end_time_minmax_interval'];
                                ?>
                    ...
                    <?php if(is_array($this->va_settings['show_form_fields']) && in_array('title', $this->va_settings['show_form_fields'])) : ?>
                        <label><strong>
                            ...
                            <?php else :  ?>
                                <?php echo $this->va_settings['title_label']; ?>
                            <?php endif; ?>
                        </strong></label>
        ...
        function va_get_time_select($name, $value = null, $id = null, $required = false, $start = false, $end = false, $interval = 0.25, $reverse = false){
            ...
            <select id="<?php echo $id; ?>" name="<?php echo $name; ?>" <?php if($required){echo 'required';} ?>>
                ...
                <?php 
                    ...
                    $times = $this->va_get_times($start_at,$end_at,$interval);
                    if($reverse){
                        arsort($times); 
                    }
                ?>
